 ...
 * <dl>
 * 		<dt>{@code FOption<R>}:</dt>
 * 		<dd>목표 상태에 도달한 경우이고, polling 과정이 종료된다.</dd>
 * 		<dt>{@code FOption.empty()}:</dt>
...
public abstract class AbstractStatePoller<R> extends PeriodicLoopExecution<R> {
	...
	protected abstract FOption<R> pollState() throws Exception;
	...
	@Override
	protected final FOption<R> performPeriodicAction(long loopIndex) throws Exception {
		return pollState();
	}
}
