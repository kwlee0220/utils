 ...
 * <p>
 * {@link #next()} 호출 당시 종료된 작업이 없는 경우에는 종료된 작업이 있을 때까지 대기하게 된다.
 * <p>
 * {@code CompletionFStream}이 close되었거나 {@link #isEndOfSupply()}가 {@code true}인 경우에는
 * 추가의 작업을 submit할 수 없으며, submit 메소드가 호출된 경우에는 {@link IllegalStateException}
 * 예외가 발생된다.
 * <p>
 * 작업 완료 콜백은 작업을 수행한 worker 쓰레드에서 동기적으로 호출되어 결과를 buffer에 적재한다.
 * 따라서 buffer가 가득 찬 상태에서는 worker 쓰레드가 {@code supply} 안에서 블록될 수 있다.
 * 작업을 수행하는 worker pool 크기와 buffer capacity의 관계에 따라 처리량이 달라질 수 있으므로
 * 둘을 함께 고려해 설정해야 한다.
 *
...
public class CompletionFStream<T> extends SuppliableFStream<Result<T>> {
	 ...
	 * <p>
	 * 작업이 종료된 후에 본 FStream이 close되거나 endOfSupply 상태가 된 경우에는
	 * 작업 수행 결과는 폐기된다.
	 * 종료된 작업 결과를 제공하는 과정에서 {@link InterruptedException}이 발생된 경우에는
	 * 인터럽트 플래그를 복원한 후 stream 전체를 에러 종료시킨다 (수집 과정 전체 중단).
	 *
	 * @param exec	작업 수행 객체
	...
	public Execution<T> submit(Execution<T> exec) {
		if ( isClosed() || isEndOfSupply() ) {
			throw new IllegalStateException("CompletionFStream is closed or end of supply");
		}
		Preconditions.checkNotNullArgument(exec, "Execution is null");

		if ( !exec.isStarted() ) {
			if ( exec instanceof StartableExecution<?> startable ) {
				startable.start();
			}
			else {
		...
		exec.whenFinished(result -> {
			try {
				...
				// 이미 close되었거나 endOfSupply 상태인 경우에는 IllegalStateException이 발생될 수 있다.
				// 이 경우는 무시한다.
			}
		});
		return exec;
	 ...
	 * <p>
	 * Callable 작업이 종료된 후에 본 FStream이 close되거나 endOfSupply 상태가 된 경우에는
	 * 작업 수행 결과는 폐기된다.
	 * 종료된 작업 결과를 제공하는 과정에서 {@link InterruptedException}이 발생된 경우에는
	 * 수집 과정 전체가 중단된다.
	 * 
	...
	public Execution<T> submit(Callable<T> callable, Executor executor) throws IllegalStateException {
		Preconditions.checkNotNullArgument(callable, "callable is null");
		if ( isClosed() || isEndOfSupply() ) {
			throw new IllegalStateException("CompletionFStream is closed or end of supply");
		}
