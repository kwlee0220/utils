 ...
 */
class MapUnorderedAsyncStream<S,T> extends AbstractFStream<Try<T>> {
	private final FStream<StartableExecution<T>> m_execStream;
	
	private final AsyncExecutionOptions m_options;
	private final Guard m_guard = Guard.create();
	@GuardedBy("m_guard") private final SuppliableFStream<Try<T>> m_outChannel;
	@GuardedBy("m_guard") int m_runningWorkerCount = 0;
	
	MapUnorderedAsyncStream(FStream<S> src, Function<? super S, ? extends T> mapper,
