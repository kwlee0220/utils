...
public class AbstractThreadedExecutionTest {
	...
	public void start_success() throws Exception {
		...
		Assert.assertEquals("ok", result.getUnchecked());
		Assert.assertEquals(AsyncState.COMPLETED, task.getState());
	}
	...
	public void start_runtime_exception_transitions_to_FAILED() throws Exception {
		...
		Assert.assertSame(cause, result.getFailureCause());
		Assert.assertEquals(AsyncState.FAILED, task.getState());
	}
	...
	public void start_executeWork_throws_InterruptedException_transitions_to_CANCELLED()
		...
		Assert.assertEquals(AsyncState.CANCELLED, task.getState());
	}
	...
	public void start_external_cancel_interrupts_and_transitions_to_CANCELLED() throws Exception {
		...
		task.start();
		Assert.assertTrue(task.awaitInExecuteWork());
		Assert.assertEquals(AsyncState.RUNNING, task.getState());
	...
	public void start_initializeThread_failure_skips_executeWork() throws Exception {
		...
		Assert.assertTrue(result.isFailed());
		Assert.assertSame(cause, result.getFailureCause());
		Assert.assertFalse(task.executeWorkCalled.get());
	}
	...
	public void start_with_injected_executor_runs_on_executor_thread() throws Exception {
		...
		try {
			...
			Assert.assertEquals("injected-pool-thread", threadNameRef.get());
		}
		finally {
			executor.shutdown();
		}
	...
	public void start_executor_rejects_transitions_to_FAILED_and_propagates() {
		...
		try {
			...
			Assert.assertSame(rejection, expected);
		}
		Assert.assertEquals(AsyncState.FAILED, task.getState());
	}

	@Test(timeout = 5_000, expected = IllegalStateException.class)
	...
	public void start_default_thread_is_daemon() throws Exception {
		...
		try {
			...
			Assert.assertTrue("default thread should be daemon", t.isDaemon());
		}
		finally {
			task.release();
			task.waitForFinished();
	...
	public void start_setDaemonThread_false_creates_user_thread() throws Exception {
		...
		try {
			Thread t = task.thread.get();
			Assert.assertNotNull(t);
			Assert.assertFalse("non-daemon flag must be honored", t.isDaemon());
		}
		finally {
			task.release();
			task.waitForFinished();
	...
	public void run_success_returns_value() throws Exception {
		ResultTask task = new ResultTask("ok");
		Assert.assertEquals("ok", task.run());
		Assert.assertEquals(AsyncState.COMPLETED, task.getState());
	}
	...
	public void run_runtime_exception_wraps_in_ExecutionException() {
		...
		try {
			...
			Assert.fail("expected ExecutionException, got " + e);
		}
		Assert.assertEquals(AsyncState.FAILED, task.getState());
	}
	...
	@Test(timeout = 5_000, expected = InterruptedException.class)
	...
	@Test(timeout = 5_000, expected = CancellationException.class)
