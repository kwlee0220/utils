...
- **`utils.http`** — OkHttp-based REST client (`HttpRESTfulClient`, `HttpClientProxy`, `OkHttpClientUtils`) with Jackson error-deserialization (`JacksonErrorEntityDeserializer`, `JacksonResponseBodyDeserializer`, `RESTfulErrorEntity`, `RESTfulRemoteException`, `RESTfulIOException`, `SpringBootErrorResponse`, `TypedServerErrorMessage`).
